...
public class AudioRecorderThread extends Thread {
    ...
    public void run() {
        if (mSampleRate != 8000 && mSampleRate != 16000 && mSampleRate != 22050
                && mSampleRate != 44100) {
            //mMediaRecorder.onAudioError(MediaRecorderBase
            // .AUDIO_RECORD_ERROR_SAMPLERATE_NOT_SUPPORT, "sampleRate not support.");
            return;
        }
        ...
        if (AudioRecord.ERROR_BAD_VALUE == mMinBufferSize) {
            //mMediaRecorder.onAudioError(MediaRecorderBase
            // .AUDIO_RECORD_ERROR_GET_MIN_BUFFER_SIZE_NOT_SUPPORT, "parameters are not supported
            // by the hardware.");
            return;
        }
        ...
        if (null == mAudioRecord) {
            //mMediaRecorder.onAudioError(MediaRecorderBase.AUDIO_RECORD_ERROR_CREATE_FAILED,
        ...
        try {
            ...
            //mMediaRecorder.onAudioError(MediaRecorderBase.AUDIO_RECORD_ERROR_UNKNOWN,
